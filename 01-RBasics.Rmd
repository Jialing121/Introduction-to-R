...
If you simply type `5*3` into the 'console' (see below) and press enter, you should receive the solution as an output which again appears in the console below your input:
...
Some other useful examples of pre-defined variables and functions are `pi`, `exp()`, `abs()`, `factorial()` and `log()` which allow use of $\pi$, $e^x$, $|x|$, $factorial(x)$, $\ln(x)$ in calculations, respectively. For example, if we wanted to calculate $e^{\ln(\pi)}$:
...
length(vec1)
length(vec2)
vec1 + vec2  #is equal to vector (1,2,3,4,1,2,3)+(1,2,3,4,5,6,7)
```
...
**Notice the comments in the above code? This can be done using the hashtag symbol and is a habit I would strongly recommend you start to implement from the off.**
